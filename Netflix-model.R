...
Stock.TS <- xts(Stock$Adj.Close,ymd(Stock$Date),drop=FALSE)
...
#Separate Test Data From Train Data
  #Define Time Intervals
    ...
    train.start <- as.Date("2015-02-02"); train.end <- as.Date("2016-06-02") #Start and end
    train.int <-  seq(train.start,train.end,"days") 
          
    test.start <- as.Date("2016-06-03"); test.end <- as.Date("2016-07-15") #start and end
...
#Run Tests on Data
  ...
  #Differencing for stationarity
  diff.mean.Train <- diff(Stock.mean.Train.t)
  ...
  bestrmse(Stock.Train,Stock.Test[1:30],p=c(1:5),d=c(1),q=c(1:5))
...
#Build Arima for transformed and not transformed
  ...
  fit.stock.train.t <- Arima(Stock.Train,order= c(1,1,1),lambda = lambda0)
  
  #without lambda
  Arima(Stock.Train,order= c(0,1,1)) #ACF:3 PACF:3
  Arima(Stock.Train,order= c(1,1,0)) #ACF:3 PACF:3
  Arima(Stock.Train,order= c(1,1,1))
  fit.stock.train <- Arima(Stock.Train,order= c(3,1,4),include.drift =TRUE, lambda = lambda0 )
  ...
  #without lambda
  Arima(Stock.mean.Train,order= c(0,1,1)) #ACF:3 PACF:3
  Arima(Stock.mean.Train,order= c(1,1,0)) #ACF:3 PACF:3
  Arima(Stock.mean.Train,order= c(1,1,1))
  fit.stock.mean.train <- Arima(Stock.mean.Train,order= c(6,1,4),include.drift = TRUE,lambda = lambda.m)
...
#BackTesting
  ...
  plot(accuracy.ts[,1],type='l',main="Forecasts vs Price",ylab="Price (Rupees)",xlab="Day") #Plot Test data
  ...
  plot(accuracy.mean.ts[,1],type='l',main="Forecasts vs Daily Mean Price",ylab="Price (Rupees)",xlab="Day") #Plot Test data
