...
#Separate Test Data From Train Data
  #Define Time Intervals
    ...
    train.start <- as.Date("2015-03-01"); train.end <- as.Date("2016-06-02") #Start and end
...
#Build Arima for transformed and not transformed
  ...
  plot.forecast((forecast.Arima(fit.stock.train,h=30)))
  lines(accuracy.ts[,1],type='l',col="red")
...
#BackTesting
  ...
  #Accuracy function
   ...
   Wtihin.Close <- forecast$mean<=Stock.high.v & forecast$mean>=Stock.low.v #Testing when forecast hits price
   Within.Close.Accuracy <- length(which(x==TRUE))/length(x) * 100
