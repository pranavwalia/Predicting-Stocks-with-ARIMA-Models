...
bestrmse<-function(train,test,p,d,q,drift) #Finds Best Model Based on RMSE
{
  min.rmse<-10^6   
  l<-length(test)
  ...
  for (i in p)
  {
    for (j in d)
    {
      for (k in q)
      {
        fit<-Arima(train,order=c(i,j,k),include.drift = drift)
...
#Build Arima for transformed and not transformed
  ...
  fit.stock.train <- Arima(Stock.Train,order= c(6,1,7),include.drift = TRUE)
