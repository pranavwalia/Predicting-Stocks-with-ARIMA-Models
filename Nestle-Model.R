...
#Separate Test Data From Train Data
  #Define Time Intervals
    ...
    train.start <- as.Date("2012-04-02"); train.end <- as.Date("2014-02-03") #Start and end
    train.int <-  seq(train.start,train.end,"days") 
          
    test.start <- as.Date("2014-02-04"); test.end <- as.Date("2014-06-07") #start and end
...
#Run Tests on Data
  ...
  bestrmse(Stock.Train,Stock.Test[1:30],p=c(1:5),d=c(0,1),q=c(1:5))
...
#Build Arima for transformed and not transformed
  ...
  fit.stock.mean.train <- Arima(Stock.mean.Train,order= c(5,0,5))
