...
bestrmse<-function(train,test,p,d,q,drift,lam) #Finds Best Model Based on RMSE
{
  min.rmse<-10^6   
  l<-length(test)
  ...
  for (i in p)
  {
    for (j in d)
    {
      for (k in q)
      {
        fit<-Arima(train,order=c(i,j,k),include.drift = drift,lambda = lam)
...
#Separate Test Data From Train Data
  #Define Time Intervals
    ...
    train.start <- as.Date("2009-03-04"); train.end <- as.Date("2016-06-02") #Start and end
    train.int <-  seq(train.start,train.end,"days") 
          
    test.start <- as.Date("2016-06-03"); test.end <- as.Date("2016-07-18") #start and end
...
#Run Tests on Data
  ...
  bestrmse(Stock.Train,Stock.Test[1:30],p=c(0:7),d=c(1),q=c(0:7),drift = TRUE)
  bestrmse(Stock.Train,Stock.Test[1:30],p=c(0:7),d=c(1),q=c(0:7),drift = TRUE,lam = lambda0)
  
  #Find bestrmse for stock.mean
  bestrmse(Stock.mean.Train,SmallerStock.mean.test,p=c(0:7),d=c(1),q=c(0:7),drift = FALSE,lam = NULL)
  bestrmse(Stock.mean.Train,SmallerStock.mean.test,p=c(7:10),d=c(1),q=c(7:10),drift = TRUE,lam=lambda.m)
...
#Build Arima for transformed and not transformed
  ...
  fit.stock.train.t <- Arima(Stock.Train,order= c(5,1,6),lambda = lambda0,include.drift = TRUE)
  ...
  fit.stock.train <- Arima(Stock.Train,order= c(5,1,6),lambda = lambda0,include.drift = TRUE)
  ...
  fit.stock.mean.train.t <- Arima(Stock.mean.Train,order= c(7,1,8),include.drift = TRUE,lambda = lambda0)
  ...
  fit.stock.mean.train <- Arima(Stock.mean.Train,order= c(7,1,7),include.drift = TRUE,lambda = lambda.m)
...
#BackTesting
  ...
  plot(accuracy.ts[,1],type='l',main="Forecasts vs Price (Costco)",ylab="Price (USD)",xlab="Day") #Plot Test data
  ...
  legend("topleft",c("Closing Price","Forecasts","High","Low"),lty=c(1,1,1,1),col=c("black","red","green","blue"),bty="n")
  
  #Full Chart (Forecasts vs Close)
  plot.forecast(forecast,sub = "Costco Close Forecast vs Close Price")
  acc.ts.pr<- array(0,dim=c(1,length(c(Stock.Train,Stock.Test[1:30]))))
  acc.ts.pr <- c(rep(NaN,times=length(Stock.Train)),accuracy.ts[,1])
  lines(acc.ts.pr,type='l',col="red")
  
  #Plot Mean Prices vs Mean forecasts
  plot(accuracy.mean.ts[,1],type='l',main="Forecasts vs Daily Mean Price (Costco)",ylab="Price (USD)",xlab="Day") #Plot Test data
  lines(accuracy.mean.ts[,2],type='l',col="red") #Plot Forecast
  lines(Stock.high.v,type='l',col="Green") #Plot high
  lines(Stock.low.v,type='l',col="Blue") #Plot Lows
  legend("topleft",c("Mean","Forecasts","High","Low"),lty=c(1,1,1,1),col=c("black","red","green","blue"),bty="n")
  
  #Full Chart (Forecasts vs Mean price)
  plot.forecast(forecast.mean,sub = "Costco Mean Forecast vs Mean Daily Price")
  acc.ts.pr<- array(0,dim=c(1,length(c(Stock.mean.Train,Stock.mean.test[1:30]))))
  acc.ts.pr <- c(rep(NaN,times=length(Stock.mean.Train)),accuracy.mean.ts[,1])
  lines(acc.ts.pr,type='l',col="red") 
  
  #Plot of Mean Forecasts vs Closing Prices
  plot(accuracy.ts[,1],type='l',main="Mean Forecasts vs Closing Price (Costco)",ylab="Price (USD)",xlab="Day") #Plot Test data
  lines(accuracy.mean.ts[,2],type='l',col="red") #Plot Forecast
  lines(Stock.high.v,type='l',col="Green") #Plot high
  lines(Stock.low.v,type='l',col="Blue") #Plot Lows
